...
tf.app.flags.DEFINE_string(
    'bottle_ckpt', './saved_model/NEW.ckpt', 'Bottleneck checkpoint file')
tf.app.flags.DEFINE_string(
    'classify_ckpt', './finetune/train3.ckpt', 'Classify Checkpoint file')
...
def main(argv=None):
    ...
    with g2.as_default():
        ...
        with slim.arg_scope([slim.conv2d], padding='VALID',
            ...
            net = slim.batch_norm(net)
            net = slim.conv2d(net,256,[3,3],stride = 2,scope='conv3')
            net = slim.batch_norm(net)
            print(net.shape)
